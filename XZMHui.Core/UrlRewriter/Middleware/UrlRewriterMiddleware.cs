...
namespace XZMHui.Core.UrlRewriter.Middleware
{
    internal class UrlRewriterMiddleware
    {
        ...
        private readonly ILogger<UrlRewriterMiddleware> _logger;

        public UrlRewriterMiddleware(
               RequestDelegate next,
               ILogger<UrlRewriterMiddleware> logger,
        ...
        public async Task Invoke(HttpContext context, IUrlRewriter urlRewriter)
        {
            ...
            if (targetUri != null)
            {
                _logger.LogInformation($"检测到流向{targetUri}的转发请求。");
                try
                {
                    var requestMessage = GenerateProxifiedRequest(context, targetUri);
                    await SendAsync(context, requestMessage);
                    _logger.LogInformation($"流向{targetUri}的转发请求完成。");
                    return;
                }
                catch (Exception ex)
                {
                    _logger.LogError($"流向{targetUri}的转发请求失败。" + ex.Message);
                    return;
                }
            }
